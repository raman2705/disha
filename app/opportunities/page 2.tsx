...
export default function OpportunitiesPage() {
  ...
  const initialCategory = searchParams.get("category") as OpportunityCategory | null;
  const initialQuery = searchParams.get("q") ?? "";
  const [query, setQuery] = useState(initialQuery);
  const [category, setCategory] = useState<OpportunityCategory | typeof allCategory>(
    initialCategory && categoryDefinitions.some((item) => item.name === initialCategory) ? initialCategory : allCategory
  );
    ...
    <div>
      ...
      <section className="mb-6 rounded-lg bg-white p-4 shadow-sm ring-1 ring-stone-200">
        <div className="grid gap-3 lg:grid-cols-[1fr_auto_auto] lg:items-center">
          ...
          <label className="flex items-center gap-2 text-sm font-semibold text-muted">
            ...
            >
              {["All", "Open", "Upcoming", "Rolling", "Closed"].map((item) => (
      ...
      <section className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-[#EEF2FF] p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Each opportunity includes access requirements, eligibility checks, readiness evidence and evaluator signals where available.</p>
        </div>
